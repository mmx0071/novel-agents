...
impl LlmClient {
        ...
        Fut: std::future::Future<Output = ()>,
    {
        ...
        tracing::info!(%model, max_tokens = tokens, deadline_secs, url = %url, "llm stream start");
        // Hard deadline — reqwest timeout alone can miss stuck body streams.
        let result = tokio::time::timeout(
            std::time::Duration::from_secs(deadline_secs),
            self.drive_chat_stream(key, body, &mut on_delta),
        )
        .await;
        let (content, tool_calls, delta_events, reasoning_content) = match result {
            Ok(Ok(v)) => v,
