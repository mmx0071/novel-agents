...
pub use tool_ui::{is_bulk_context_tool, tool_output_for_ui};
...
use novelx_pipeline::{
    active_volume_for_chapter, bound_for_volume, build_chapter_context, check_plot_write_gate_with,
    confirm_setup_approve, confirm_setup_revise, design_plot_force_allowed, ensure_bridge_plot_active,
    ensure_plot_card_lifecycle_frontmatter, execute_pipeline, list_plots_summary, list_projects,
    load_memory, load_meta_json, load_volume_bounds, lock_brief, lore_query,
    mark_volume_sync_skipped, materialize_plot_card_markdown, maybe_advance_setup_after_outlines,
    plot_design_blocked_reason, read_arc_outline_excerpt, read_chapter_draft, read_world_doc,
    rebuild_plot_index, resolve_setup_phase, resolve_volume_phase, run_volume_audit, run_volume_sync,
    set_volume_phase, steer_revision_options, update_plot_card, validate_arc_outline, validate_bible,
    validate_entity_card, validate_master_outline, validate_plot_card, volume_chapter_span,
    ContextProfile, EntityKind, PhaseEnforceFlags, PlotWriteGate, PlotWriteMode, RevisionOptions,
    RunMode, SetupPhase, VolumePhase,
};
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        ...
        let as_bridge = match check_plot_write_gate_with(&dir, enforce) {
            PlotWriteGate::Block { message } => {
                let handoff = message.contains("卷间交接") || message.contains("design_plot");
                return Ok(ToolResult {
                    output: format!("⛔ 写章已拦截\n\n{message}"),
                    data: json!({
                        "blocked": true,
...
impl ToolHandler for AuditChapter {
    ...
    fn description(&self) -> &'static str {
        "只审校不改正文。未通过时会引导用户选择：按审校局部修订 / 接受问题 / 其他。"
    }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        ...
        .await?;
        // Prefer full model report in the ToolCall card (Codex-style └ output).
        let output = if ctx.progress.is_some() {
            if !run.message.is_empty() {
                format!("\n——\n{}", run.message)
            } else {
...
async fn run_audit_queue_until_gate(
    ...
    loop {
        ...
        let passed = run.consistency_passed == Some(true) && !run.needs_user_choice;
        let short = if passed {
            "一致性通过".to_string()
        } else {
            "一致性未通过".to_string()
...
impl ToolHandler for ApplyDraftPatch {
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        // Delegate to revise with local-only
...
impl ToolHandler for DesignEntity {
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        ...
        let body = if path.exists() && brief.is_empty() {
            std::fs::read_to_string(&path)?
        } else {
            let generated = generate_entity_card(ctx, project, kind, name, brief, role).await?;
            ...
            let audit = run_setting_audit(ctx, project, "拟新增实体卡", &generated).await?;
...
impl ToolHandler for DesignPlot {
    ...
    fn description(&self) -> &'static str {
        "新增剧情卡到 plots/（须已有卷纲；有 in_progress/bridging 或欠衔接章时拒绝，避免叠卡）"
    }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        ...
        if arc_excerpt.is_none() && !force {
            return Ok(ToolResult {
                output: format!(
                    "未找到可用卷纲 artifacts/arc_outline.md，请先 design_arc_outline 再 design_plot。\
                     （紧急跳过可传 force=true）"
                ),
                data: json!({"blocked": true, "reason": "missing_arc_outline"}),
            });
        }
        if !force {
            if let Some(reason) = plot_design_blocked_reason(&dir) {
                return Ok(ToolResult {
                    output: reason,
                    data: json!({"blocked": true, "reason": "active_plot_or_pending_bridge"}),
                });
            }
        }
        ...
        .await?;
        let body = match validate_plot_card(&body) {
            Ok(n) => n,
            Err(e) => {
                return Ok(ToolResult {
                    output: format!("剧情卡格式不合规，未写入：{e}"),
                    data: json!({"blocked": true, "schema_error": e.to_string()}),
                });
            }
        };
        let audit = run_setting_audit(ctx, project, "拟新增剧情卡", &body).await?;
        if audit.blocker && !force {
            return Ok(ToolResult {
                output: format!("设定审计 BLOCKER，未写入：{}\n{}", audit.summary, audit.report),
                data: json!({"blocked": true, "audit": audit.raw}),
        ...
        let vol = act.unwrap_or(1) as u32;
        let _ = ensure_plot_card_lifecycle_frontmatter(&path, vol, "planned");
        let _ = rebuild_plot_index(&dir);
        Ok(ToolResult {
            output: format!("已写入剧情卡 {}（status=planned，已入 plots/index.json）", path.display()),
            data: json!({"path": path, "title": title, "audit_summary": audit.summary}),
        })
    }
}
...
impl ToolHandler for ListPlots {
    ...
    fn description(&self) -> &'static str {
        "列出剧情卡（可按卷/状态过滤），含 lifecycle 与卷 active_main"
    }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        ...
        let lines: Vec<String> = plots
            ...
            .map(|p| {
                format!(
                    "v{} | {} | {} | {} | needs_bridge={} bridge_done={} | next={}",
                    p["volume_index"].as_u64().unwrap_or(0),
                    p["status"].as_str().unwrap_or("?"),
                    p["plot_type"].as_str().unwrap_or("?"),
                    p["title"].as_str().unwrap_or("?"),
                    p["needs_bridge"].as_bool().unwrap_or(false),
                    p["bridge_done"].as_bool().unwrap_or(false),
...
impl ToolHandler for UpsertSetting {
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        ...
        if is_nomenclature_topic(topic) {
            return upsert_nomenclature_setting(ctx, &dir, project, content, force).await;
        }
        ...
        let audit =
            run_setting_audit(ctx, project, &format!("拟更新 Bible·{topic}"), &section).await?;
        if audit.blocker && !force {
            return Ok(ToolResult {
                output: format!("设定审计 BLOCKER，未写入：{}\n{}", audit.summary, audit.report),
                data: json!({"blocked": true, "audit": audit.raw}),
        ...
        Ok(ToolResult {
            ...
            data: json!({"path": path, "topic": topic, "chars": merged.chars().count()}),
        })
    }
}
...
impl ToolHandler for DesignMasterOutline {
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        ...
        let prompt = format!(
            "为小说《{}》（题材：{}，目标{}章）撰写/更新总纲 Markdown。\
             含：一句话卖点、三幕结构、主角弧、主线冲突、中后期升级台阶。\
             用户补充：{brief}\n\n现有总纲（可空）：\n{existing}",
            state.name, state.genre, state.target_chapters
        );
        let body = ctx
            .llm
            .complete(
            ...
            .await?;
        let text = match validate_master_outline(body.trim()) {
            Ok(n) => n,
            Err(e) => {
                return Ok(ToolResult {
...
impl ToolHandler for DesignArcOutline {
    ...
    fn description(&self) -> &'static str {
        "生成/更新当前卷纲（artifacts/arc_outline.md）"
    }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
            "properties":{
                "project":{"type":"string"},
                "arc":{"type":"integer","description":"卷第，默认 1"},
                "brief":{"type":"string"}
            },
            "required":["project"]
        })
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        let project = args["project"].as_str().unwrap_or("");
        let arc = args["arc"].as_u64().unwrap_or(1);
        let brief = args["brief"].as_str().unwrap_or("");
        if project.is_empty() {
            anyhow::bail!("project 必填");
        }
        ...
        let body = ctx
            ...
            .await?;
        let text = match validate_arc_outline(body.trim()) {
            Ok(n) => n,
            Err(e) => {
                return Ok(ToolResult {
        ...
        if bounds.is_empty() {
            if let Some(mut b) = bound_for_volume(&load_volume_bounds(&dir), arc as u32) {
                b.volume_index = arc as u32;
                bounds.push(b);
            }
        }
        for b in &bounds {
            if b.volume_index == arc as u32 || bounds.len() == 1 {
                let _ = novelx_pipeline::sync_act_chapter_bounds(&dir, b);
            }
        }
        let cond_n = bounds
            .iter()
            .find(|b| b.volume_index == arc as u32)
        ...
        Ok(ToolResult {
            output: format!(
                "已写入卷纲 {}（终止条件 {} 条）；setup_phase={}；volume_phase={}",
                path.display(),
                cond_n,
                setup.as_str(),
                vol_phase.as_str()
                ...
                while let Some(ev) = rx.recv().await {
                    match ev {
                        ...
                        novelx_pipeline::PipelineEvent::StepCompleted { agent, summary } => {
                            let _ = p.send(format!("✓ {agent}: {summary}\n"));
                        }
...
async fn generate_plot_card(
    ...
    let prompt = format!(
        "为《{project}》写剧情卡「{title}》。要点：{brief}\n\
         {arc_block}\n\
         剧情卡必须服从卷纲节点阶梯与终止条件；不得另起与卷纲冲突的主线。\
         ...
         status=planned；含 volume_index/needs_bridge/next_plot/fields。"
    );
    let body = ctx
        .llm
        .complete(
...
fn load_agent_skill(ctx: &ToolContext, name: &str) -> String {
    let root = ctx.config_root.join("skills/agents");
...
async fn run_setting_audit(
    ...
    let dir = project_dir(&ctx.projects_root, project);
    let (_, bible) = read_world_doc(&dir);
    let skill = load_agent_skill(ctx, "setting-auditor");
    let prompt = format!(
        "审计对象：{label}\n\n## 拟写入内容\n{}\n\n## 现有 Bible 节选\n{}\n\n\
         只输出 JSON：passed/summary/conflicts（含 severity）。",
        candidate.chars().take(3500).collect::<String>(),
    ...
    let raw_text = ctx
        ...
        .complete(
            &skill,
            &prompt,
            Some(&ctx.llm.model_for_agent("setting_auditor")),
        )
        .await
        .unwrap_or_else(|_| {
            json!({"passed":true,"summary":"审计跳过（模型失败）","conflicts":[]}).to_string()
        });
    let v = extract_json_value(&raw_text).unwrap_or(json!({
        "passed": true,
        "summary": raw_text.chars().take(200).collect::<String>(),
        "conflicts": []
    }));
    // Only block on explicit BLOCKER severity.
    let blocker = v
        .get("conflicts")
    ...
    Ok(SettingAuditResult {
        ...
        summary: v
            ...
            .unwrap_or("")
            .to_string(),
        report: raw_text.chars().take(1200).collect(),
        raw: v,
    })
}
...
async fn upsert_nomenclature_setting(
    ...
    let body = if content.trim().chars().count() > 80 {
    ...
    let audit = run_setting_audit(ctx, project, "拟更新名词表", &body).await?;
    if audit.blocker && !force {
        return Ok(ToolResult {
            output: format!("设定审计 BLOCKER，未写入：{}\n{}", audit.summary, audit.report),
            data: json!({"blocked": true, "audit": audit.raw}),
...
impl ToolHandler for AuditSetting {
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        ...
        let audit = run_setting_audit(ctx, project, focus, &pack).await?;
        Ok(ToolResult {
            output: format!(
                "设定审计：{}\nblocker={}\n{}",
...
impl ToolHandler for ConfirmSetup {
    ...
    fn description(&self) -> &'static str {
        "确认或打回灵感定稿：approve→ready 可写章；revise→collecting 重新出纲"
    }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
...
impl ToolHandler for SteerRun {
    ...
    fn description(&self) -> &'static str {
        "接续审校门控：revise（局部修订）/ accept（接受问题；接受后需再 audit）"
    }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
            "properties":{
                "project":{"type":"string"},
                "chapter":{"type":"integer"},
                "choice":{"type":"string","description":"revise | accept（亦兼容中文「按审校局部修订」「接受问题」）"},
                "instructions":{"type":"string"}
            },
    ...
    async fn call(&self, ctx: &ToolContext, args: Value) -> Result<ToolResult> {
        ...
        if accept {
            return Ok(ToolResult {
                output: "已记录接受问题。请手动修复后调用 audit_chapter 复审；未自动推进发布。".into(),
                data: json!({"accepted": true, "chapter": chapter}),
            });
        }
        let audit_path = dir
            .join("chapters")
        ...
        let mut rev = steer_revision_options(&instructions);
        rev.audit_issues = issues;
        rev.prefer_local_patch = true;
        rev.revision_mode = true;
        // Always in-process — spawn/wait_agent stranded parent turns after child publish_result.
        let run = run_pipeline_streaming(ctx, project, chapter, RunMode::Revise, rev).await?;
        let output = if ctx.progress.is_some() {
            if !run.message.is_empty() {
                format!("\n——\n{}", run.message)
...
impl ToolHandler for ActivateAgents {
    ...
    fn description(&self) -> &'static str {
        "持久化激活/停用流水线 Agent（写入 state.active_agents）"
    }
    fn parameters(&self) -> Value {
        json!({
            "type":"object",
        ...
        while let Some(ev) = rx.recv().await {
            ...
            let chunk = match ev {
                ...
                PipelineEvent::LlmDelta { agent, delta } => {
                    match classify_llm_progress_delta(&delta) {
                        ...
                        LlmProgressKind::Prose => {
                            // Writer with draft.md flush: only「正文已写入」lines.
                            if draft_backed {
                                continue;
                            }
                            let n = {
                        ...
                        LlmProgressKind::OtherStatus(s) => s,
                    }
                }
                PipelineEvent::DraftFlushed { chars } => {
                    ...
                    // Quiet chat: report roughly every +400 chars (reader still refreshes).
                    if last_draft_chars > 0 && chars.saturating_sub(last_draft_chars) < 400 {
                        continue;
                    }
                    last_draft_chars = chars;
                    format!("  正文已写入 · {chars}字\n")
                }
...
fn classify_llm_progress_delta(delta: &str) -> LlmProgressKind {
    ...
    if t.starts_with('（') {
        return LlmProgressKind::OtherStatus(format!("{t}\n"));
    }
    LlmProgressKind::Prose
}
